    ...
    def __post_init__(self):
        if self.required_information is None:
            ...
            self.required_information = [
                ...
                {
                    "field": "concerns",
                    "question": "What are your specific concerns?",
                },
                {
                    "field": "style_guide",
...
class QueryEnricher:
    ...
    def _create_enrichment_chain(self):
        ...
        questions_text = "\n".join(
            [
                f'  * "{info["question"]}"'
                for info in self.config.required_information or []
            ]
        )
        ...
        enrichment_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    f"""You are a query refinement assistant for
...
Rules:
- clarifications: MUST include questions for ANY of these that are NOT explicitly
mentioned in the query:
{questions_text}
  IMPORTANT: Keep the clarifications list populated until ALL 6
  items above are provided.
  Even if you have PR ID, repository, and branch, you MUST still ask for concerns,
  style guide, and test requirements.
...
NEVER set clarifications to empty [] unless ALL 6 required items are explicitly
    ...
    def _create_rewrite_chain(self):
        ...
        rewrite_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "Rewrite the information into a single, natural, "
                    "well-formed question. Be concise and clear.",
                ),
                (
                    "user",
...
class EnrichmentSession:
    ...
    def run(self, initial_question: str) -> tuple[str, dict[str, Any]]:
        ...
        while self.round_num < self.enricher.config.max_rounds:
            ...
            if new_answers:
                ...
                current_query = (
                    self.initial_question
                    + " | "
                    + " | ".join(self.provided_information)
                )
                ...
                user_continue = (
                    input("Continue with remaining clarifications? (yes/no): ")
                    .strip()
                    .lower()
                )
        ...
        natural_question = self.enricher.generate_natural_question(
            self.initial_question, self.provided_information
        )
