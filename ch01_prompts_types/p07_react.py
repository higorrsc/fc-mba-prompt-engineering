...
Do not fabricate any information that is not provided in the context. Example: if the
context does not provide errors logs, do not use error logs in your reasoning.
...
func CreateProduct(w http.ResponseWriter, r *http.Request) {
    ...
    stmt, err := db.Prepare("INSERT INTO products (id, name, description, price, stock)
    VALUES (?, ?, ?, ?, ?)")
    if err != nil {
        log.Fatal(err)
    }
...
MSG_2 = """
You are a travel planner helping a family choose the best way to go from Orlando
...
- Options:
  - **Flight**: $220 per person round trip, 3-hour flight,
  plus $80 total in baggage fees.
  - **Train**: $150 per person round trip, 20-hour journey,
  with onboard WiFi and beds available for $50 extra per person.
  - **Car rental**: $60/day, 2 days of driving each way (gas
  + tolls estimated $250 total). Kids get restless on long trips.

Other details:
- The kids’ school finishes on July 9 at noon.
- Parents prefer not to arrive too tired, since they have a family
wedding on July 11 in the morning.
