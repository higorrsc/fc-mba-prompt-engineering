...
1. Start by listing the subproblems that need to be solved as a to-do list.
Use markdown checkboxes: [ ] for pending, [x] for completed.
2. As you solve each subproblem, update the to-do list by marking it as [x]
and write the solution right below it.
3. Continue until all items are solved.
4. At the end, combine all the solutions into a final integrated design for
the URL shortener.
...
result = model.invoke(MSG_1)
print_llm_result(MSG_1, result)
