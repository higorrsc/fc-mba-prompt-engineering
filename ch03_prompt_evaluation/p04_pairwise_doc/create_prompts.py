...
def load_yaml_as_chat_template(filename):
    """Load YAML file as ChatPromptTemplate."""

    with open(
        os.path.join(script_dir, f"prompts/{filename}"),
        "r",
        encoding="utf-8",
    ) as f:
...
if __name__ == "__main__":
    push_prompt(
        ...
        PROMPT_A_OBJ,
        description=(
            "Prompt A - Generates structured technical "
            "documentation from multiple Python files."
        ),
    )
    ...
    push_prompt(
        ...
        PROMPT_B_OBJ,
        description=(
            "Prompt B - Generates narrative documentation "
            "explaining project functionality and decisions."
        ),
    )
